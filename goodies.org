...
=when-let= defined here has the same functionality, as [[https://common-lisp.net/project/alexandria/draft/alexandria.html][alexandria:when-let*]].
...
* needs
=needs= macro's philosophy is similar to =deftest= from 
