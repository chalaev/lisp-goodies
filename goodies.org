#+TITLE: my eLisp macros and functions used in different projects
#+AUTHOR: Oleg Shalaev
...
#+END_SRC
– I should probably get rid of =sleep-for= here by adding "&" at the end, similarly to how it is done in =dired-do-async-shell-command=.
...
(defun parse-date-time(str)
  (if (string-match "[0-9]\\{4\\}-[0-9][0-9]-[0-9][0-9] [0-9][0-9]:[0-9][0-9]" str)
    ...
    (let ((SS (split-string str)))
      (append (parse-time (cadr SS))
	      (parse-date (car SS))))))
#+END_SRC
#+BEGIN_SRC emacs-lisp :results drawer
(list (parse-time "16:09:37")
(format-time-string "%F %H:%M:%S %Z" (apply #'encode-time (parse-date-time "10/21 14:54:40"))))
#+END_SRC

#+RESULTS:
:RESULTS:
((37 9 16) 2020-10-21 14:54:40 EDT)
:END:

#+BEGIN_SRC emacs-lisp :results drawer
(mapcar #'(lambda(x) (format-time-string "%F %H:%M:%S %Z" (apply #'encode-time (parse-date-time x))))  (list "10/21 14:54:40" "2020-10-10 14:54:40  EDT"))
#+END_SRC

#+RESULTS:
:RESULTS:
(2020-10-21 14:54:40 EDT 2020-10-10 14:54:40 EDT)
:END:
