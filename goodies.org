...
#+END_SRC
← *probably I should replace macroexpand-1 → eval* in all macro definitions.
...
CL code is the same:
#+BEGIN_SRC lisp :tangle generated/macros.lisp :shebang ";; generated from https://notabug.org/shalaev/lisp-goodies/src/master/goodies.org"
(defmacro when-let (vars &rest body)
  "when with let using standard let-notation"
  (if (caar vars)
  `(let ((,(caar vars) ,(cadar vars)))
...
#+BEGIN_SRC lisp :tangle generated/files.lisp :shebang ";; generated from https://notabug.org/shalaev/lisp-goodies/src/master/goodies.org"
(defun safe-mkdir (dirname)
