...
#+BEGIN_SRC lisp :tangle generated/tests.lisp :shebang ";; -*-  lexical-binding: t; -*-"
...
* when-let
** Emacs Lisp
=when-let= and =when-let*= are defined in ~subr-x.el~:
#+BEGIN_SRC emacs-lisp :tangle generated/macros.el
(require 'subr-x)
#+END_SRC
...
* if-let
** Emacs Lisp
Similarly to [[when-let]], =if-let= is defined in ~subr-x.el~.
