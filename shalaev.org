...
("^(\\p{Script:Latin}+)\\s*=\\s*([^\\s]+)\\s*$" (cons (aref matches 0) (aref matches 1))); one-word parameter
("^(\\p{Script:Latin}+)\\s*=(.*)$" (cons (aref matches 0) (string-trim '(#\Space #\Tab) (aref matches 1)))); sentence
(otherwise (warn "conf line ignored: ~s" str))))))))
#+END_SRC
...
*** Common Lisp
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
(defmacro split-string(str)
  `(split-sequence:split-sequence #\Space ,str))
(defmacro substring(str index)
  `(subseq ,str ,index))
(defun parse-parameter(str &optional par-type)
"for =parse-conf=: optionally parses first (string) argument into the specified type"
  (when str
    (cond
     ((eql par-type 'integer) (parse-integer str))
#+END_SRC
← =integer= is the only recognized type as of 02/23; we might need more special type treatment here.
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
     ((consp par-type) (mapcar #'(lambda(s)(parse-parameter s (car par-type))) (split-string str)))
...
(let((TO(typeof(car CV))))
  `(cons (quote ,(cadr CV))
	 ,(if-let((cCV(caddr CV)))
	      `(or(parse-parameter(cdr(assoc  ,(cadr CV) ,conf :test #'string=)) (quote ,TO)) ,cCV)
	    `(parse-parameter(cdr(assoc ,(cadr CV) ,conf :test #'string=)) (quote ,TO)))))
#+END_SRC
if =(length CV)= ⇒ 2, type =TO= is either specified explicitly (when =(KP(car CV))= ⇒ t), or is determined from the default value:
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
(let((TO(typeof-expr(car(if(KP(car CV)) CV (cdr CV)))))
  ...
  `(cons (quote ,varName)
	 ,(if default
	      `(or(parse-parameter(cdr(assoc ,varName ,conf :test #'string=)) (quote ,TO)) ,default)
	    `(parse-parameter(cdr(assoc ,varName ,conf :test #'string=)) (quote ,TO))))))
#+END_SRC
if =(listp CV)= ⇒ nil:
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
`(cons (quote ,CV) (parse-parameter(cdr(assoc ,CV ,conf :test #'string=))))))
	 vars)))))
#+END_SRC
Usage examples:
1. simplest case: =(parse-conf cc (a b))= ⇒ (expands to)
#+BEGIN_SRC lisp :results drawer
(macroexpand-1 '(shalaev/conf::parse-conf cc ("a" "b")))
#+END_SRC
...
(LET ((#:|conf692| CC))
  (LIST
   (CONS '"a"
         (SHALAEV/CONF::PARSE-PARAMETER
          (CDR (ASSOC "a" #:|conf692| :TEST #'STRING=))))
   (CONS '"b"
         (SHALAEV/CONF::PARSE-PARAMETER
          (CDR (ASSOC "b" #:|conf692| :TEST #'STRING=))))))
T
:END:

2. specifying value type =(parse-conf cc ((:integer a) ((:integer) b)))= is useful when no default value supplied ⇒
#+BEGIN_SRC lisp :results drawer
(macroexpand-1 '(shalaev/conf::parse-conf cc ((:integer "a") ((:integer) "b"))))
#+END_SRC
...
(LET ((#:|conf693| CC))
  (LIST
   (CONS '"a"
         (SHALAEV/CONF::PARSE-PARAMETER
          (CDR (ASSOC "a" #:|conf693| :TEST #'STRING=)) ':INTEGER))
   (CONS '"b"
         (SHALAEV/CONF::PARSE-PARAMETER
          (CDR (ASSOC "b" #:|conf693| :TEST #'STRING=)) '(:INTEGER)))))
T
:END:
...
3. specifying default values (used unless contained in =cc=): =(parse-conf cc ((a 1) (c 3)))=  ⇒
   allows to supply type to =parse-parameter=:
#+BEGIN_SRC lisp :results drawer
(macroexpand-1 '(shalaev/conf::parse-conf cc (("a" 1) ("c" 3))))
#+END_SRC
...
(LET ((#:|conf695| CC))
  (LIST
   (CONS '"a"
         (OR
          (SHALAEV/CONF::PARSE-PARAMETER
           (CDR (ASSOC "a" #:|conf695| :TEST #'STRING=)) ':INTEGER)
          1))
   (CONS '"c"
         (OR
          (SHALAEV/CONF::PARSE-PARAMETER
           (CDR (ASSOC "c" #:|conf695| :TEST #'STRING=)) ':INTEGER)
          3))))
T
:END:
...
4. sometimes, when default value is a variable, both type and default value must be specified:
#+BEGIN_SRC lisp :results drawer
(macroexpand-1 '(shalaev/conf::parse-conf cc ((:integer "abc" xy))))
#+END_SRC
...
(LET ((#:|conf697| CC))
  (LIST
   (CONS '"abc"
         (OR
          (SHALAEV/CONF::PARSE-PARAMETER
           (CDR (ASSOC "abc" #:|conf697| :TEST #'STRING=)) ':INTEGER)
          XY))))
T
:END:
...
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
(defmacro let-conf(configuration vars &body body)
"untested"
  `(let((,(car configuration) (parse-conf ,(cdr configuration) ,vars)))
,@body))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle generated/conf.lisp
(defmacro letc(conf-data vars &body body)
"untested"
(let((conf-var(s-gensym)))
  `(let((,conf-var (parse-conf ,conf-data ,vars)))
(let(,@(mapcar (lambda(v)
(let((VN(ifn(listp v)v(if(KP(car v))(cadr v)(car v)))))
`(,VN (cdr(assoc  ,VN ,conf-var :test #'string=)))))
vars))
,@body))))
#+END_SRC
