...
(print (macroexpand-1 
  '(when-let ((x 1) (y (1+ x)))
       (s-incf x y) (s-incf y x))))
#+END_SRC

#+RESULTS:
:RESULTS:
(let ((x 1)) (when x (let ((y (1+ x))) (when y (s-incf x y) (s-incf y x)))))
:END:
...
*The following is going to be my general (standard) way of reporting results of a file operation both in EL and CL:*
...
I load the resulting [[file:generated/local-packages.el][local-packages.el]] from my [[file:generated/dot.emacs][~/.emacs]] and assume that whoever uses [[https://github.com/search?q=user%3Achalaev+language%3A%22Emacs+Lisp%22&type=Repositories&ref=advsearch&l=Emacs+Lisp&l=][my lisp packages]] does the same thing.

Defining important directories:
#+BEGIN_SRC emacs-lisp :tangle generated/local-packages.el :shebang ";; -*- mode: Emacs-Lisp;  lexical-binding: t; -*-"
...
So in [[file:Makefile][Makefiles]] I often use ~printangle~:
#+BEGIN_SRC emacs-lisp :tangle generated/make.el
(defun printangle(FN)
  "to be used in Makefile instead of org-babel-tangle-file"
  (let ((l (length default-directory)))
    (apply #'concat (mapcar #'(lambda(x) (substring (format "%s " x) l)) (org-babel-tangle-file FN)))))
#+END_SRC
(together with =xargs= for unquoting emacs'es output).
